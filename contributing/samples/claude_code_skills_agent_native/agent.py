...
"""Native-tools version of the Claude Code skills sample.

This sample intentionally does not configure a code executor and removes
`run_skill_script` from the skill toolset. Skills are limited to reviewed,
purpose-built tools exposed by the application itself.

Runtime configuration:
  - `ADK_NATIVE_SKILLS_PROJECT_ROOT` limits file tools to a single repo.
  - `ADK_NATIVE_SKILLS_MAX_TEXT_BYTES` caps file and diff payload size.
  - `ADK_NATIVE_SKILLS_GIT_TIMEOUT_SECONDS` bounds git subprocess calls.
"""
...
_PROJECT_ROOT_ENV = 'ADK_NATIVE_SKILLS_PROJECT_ROOT'
_MAX_TEXT_BYTES_ENV = 'ADK_NATIVE_SKILLS_MAX_TEXT_BYTES'
_GIT_TIMEOUT_ENV = 'ADK_NATIVE_SKILLS_GIT_TIMEOUT_SECONDS'
_BLOCKED_FILE_NAMES = {
...
def _ensure_safe_project_path(path: Path) -> None:
    ...
    if name in _BLOCKED_FILE_NAMES or path.suffix.lower() in _BLOCKED_SUFFIXES:
        raise PermissionError(f'access to {path.name!r} is blocked')
...
def lint_project_file(path: str) -> dict[str, Any]:
    """Return simple lint metrics for a project file without running shell scripts."""
    try:
        target = _resolve_project_path(path)
        text = _read_text_file(target)
...
def count_python_complexity(path: str) -> dict[str, Any]:
    """Return basic Python structure metrics for a file under project root."""
    try:
        target = _resolve_project_path(path)
        source = _read_text_file(target)
        ...
        return {
            ...
            'long_functions': [
                ...
                if (func.end_lineno or 0) - func.lineno > 50
            ],
...
def write_project_file(path: str, content: str) -> dict[str, Any]:
    ...
    Prefer `replace_text_in_project_file` for targeted changes. Use this
    tool when you need to replace the full file contents.
    """
    try:
        target = _resolve_project_path(path)
...
root_agent = Agent(
    ...
    name='claude_code_skills_agent_native',
    description=(
        'An agent that uses local Claude-style skills backed by reviewed '
        'native tools instead of script execution.'
    ),
    tools=[skill_toolset],
)
